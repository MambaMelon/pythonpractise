...
for i in range(31):
    file = open(r'F:\dd\20181108_offline\%s.csv' % (i+1))
    for line in file:
        arr = line.replace('\n', '').split(',')
        op = int(arr[2]) - 1
        statitic[i][op] += 1
...
# 画出折线图
yahei = matplotlib.font_manager.FontProperties(fname='C:/Windows/Fonts/simhei.ttf')
...
# 构建商品子集
item_set = []
file = open(r'F:\dd\20181108_offline\tianchi_fresh_comp_train_item.csv')
file.readline()
for line in file:
    arr = line.replace('\n', '').split(',')
...
for i in range(31):
    ...
    for line in file:
        ...
        if arr[1] in item_set:
            lines.append(line)
    file.close()
    file = open(r'F:\dd\20181108_offline\%s.csv' % (i + 1), 'w')
    for line in lines:
        file.write(line)
    file.close()
